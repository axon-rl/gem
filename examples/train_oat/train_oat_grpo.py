...
class Actor(PPOActor):
    ...
    def step(
        ...
        info["actor/mean_episode_success"] = np.mean(
            [episode[-1].rewards == 1 for episode in flattened_episodes]
    ...
    def agent_act(
        self, vec_observation: List[str], prompt_template: str
    ) -> Tuple[str, dict]:
            ...
            for i, step_data in enumerate(episode):
                ...
                transitions.append(
                    TransitionData(
                        ...
                        info={
                            "actor/action_is_formatted": step_data.action_is_formatted,
                            "actor/step_reward": step_data.rewards,
                            "actor/discount_factor": self.args.gamma,
    ...
    def run_eval_episode(
        ...
        try:
            ...
            dataset = DummyPromptDataset(size=self.args.eval_n_if_no_dataset)
...
class Learner(PPOLearner):
    def _init(self, args: Args, actors: List[Actor]) -> None:
        ...
        self.masked_aggregator = (
            functools.partial(
                masked_sum,
                constant_normalizer=args.length_norm_constant
                or args.generate_max_length,
            )
