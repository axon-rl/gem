...
import abc
from typing import Any, Dict, Iterator, Optional, SupportsFloat, Tuple
...
class AECEnv(MultiAgentEnv):
    ...
    def last(self, observe: bool = True) -> Tuple[str, float, bool, bool, Dict[str, Any]]:
    ...
    def _was_dead_step(self, action: Optional[Any]) -> bool:
        ...
        return (
            self.terminations.get(agent, False) or 
...
class AECIterable:
    ...
    def __next__(self) -> str:
        ...
        if (self.env.terminations.get(agent, False) or 
            ...
            if all(
                self.env.terminations.get(a, False) or 
                self.env.truncations.get(a, False) 
