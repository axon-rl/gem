...
from gem.multiagent.agents import BaseAgent, UserAgent, ToolAgent
...
class TestToolAgent:
    ...
    def test_parse_tool_request_json_format(self):
        ...
        request = agent._parse_tool_request('{"tool": "search", "parameters": {"query": "test"}}')
        assert request is not None
        assert request["tool"] == "search"
        assert request["parameters"]["query"] == "test"
    ...
    def test_execute_tool(self):
        ...
        result = agent._execute_tool({"tool": "search", "parameters": {"query": "test"}})
    ...
    def test_act_with_tool_request(self):
        ...
        tools = {
            "search": lambda query="": f"Results: {query}"
        }
